...
func (h *ComputeHandler) InstanceDeactivate(event *revents.Event, cli *v3.RancherClient) error {
	...
	response, err := constructDeploymentSyncReply(utils.IsNoOp(event), request.InspectIp, request.Containers[0], request.Containers[0].ExternalId, h.dockerClient, h.cache, networkKind, nil)
	if err != nil {
