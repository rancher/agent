...
func setupPublishPorts(hostConfig *container.HostConfig, containerSpec v3.Container) {
	hostConfig.PublishAllPorts = containerSpec.PublishAllPorts
}

func setupDNSSearch(hostConfig *container.HostConfig, containerSpec v3.Container) error {
...
func setupNetworking(containerSpec v3.Container, config *container.Config, hostConfig *container.HostConfig, idsMap map[string]string, networkKind string) error {
	portsSupported, hostnameSupported, err := setupNetworkMode(containerSpec, networkKind, config, hostConfig, idsMap)
	if err != nil {
...
func setupNetworkMode(containerSpec v3.Container, networkKind string, config *container.Config, hostConfig *container.HostConfig, idsMap map[string]string) (bool, bool, error) {
	/*
...
func setupFieldsHostConfig(fields v3.Container, hostConfig *container.HostConfig) {
...
func setupComputeResourceFields(hostConfig *container.HostConfig, containerSpec v3.Container) {
...
func setupRancherFlexVolume(volumes []v3.Volume, dataVolumes []string, progress *progress.Progress) ([]string, error) {
	bindMounts := []string{}
...
func unmountRancherFlexVolume(volumes []v3.Volume) error {
	for _, volume := range volumes {
		if IsRancherVolume(volume) {
