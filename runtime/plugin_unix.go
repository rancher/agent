...
func callRancherStorageVolumeAttach(volume v2.Volume) error {
...
func callRancherStorageVolumePlugin(volume v2.Volume, action string, payload interface{}) (Response, error) {
	transport := new(http.Transport)
	sockets.ConfigureTransport(transport, "unix", rancherStorageSockPath(volume))
	client := &http.Client{
		Transport: transport,
